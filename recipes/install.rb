...
if File.exists?("#{node['magento']['dir']}/install.php")
  if not_installed?
    url = URI::HTTP.build({:host=>node['magento']['server']['servername'],:port=>Integer(node['magento']['server']['unsecure_port']),:path=>'/'})
    secure_url = URI::HTTPS.build({:host=>node['magento']['server']['servername'],:port=>Integer(node['magento']['server']['secure_port']),:path=>'/'})
    install =   <<-EOH
    php -f install.php -- \
